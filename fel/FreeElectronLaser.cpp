...
KOKKOS_INLINE_FUNCTION void serial_for(callable c, ippl::Vector<uint32_t, Dim> from,
                                       ippl::Vector<uint32_t, Dim> to, Ts... args) {
...
KOKKOS_INLINE_FUNCTION void scatterLineToGrid(const ippl::NDIndex<3>& ldom, view_type& Jview,
                                              ...
                                              const scalar factor, int nghost = 1) {
    ...
    if (from_grid.first[0] == to_grid.first[0] && from_grid.first[1] == to_grid.first[1]
        ...
        scatterToGrid(ldom, Jview, hr, origin, ippl::Vector<scalar, 3>((from + to) * scalar(0.5)),
    ...
    scatterToGrid(ldom, Jview, hr, origin, ippl::Vector<scalar, 3>((from + relay) * scalar(0.5)),
...
namespace ippl {
    ...
    struct Bunch_eb : public ippl::ParticleBase<PLayout> {
        ...
        ippl::ParticleAttrib<ippl::Vector<ippl::Vector<scalar, 3>, 2>>
            EB_gather;  // Electric field container for particle gathering
    };
...
KOKKOS_INLINE_FUNCTION typename View::value_type gather_helper(const View& v,
...
scalar test_gauss_law(uint32_t n) {
    ...
    ippl::Vector<scalar, 3> extents{meter_in_unit_lengths, meter_in_unit_lengths,
    ...
    ippl::Vector<scalar, 3> origin = {scalar(-0.5 * meter_in_unit_lengths),
    ...
    ippl::NSFDSolverWithParticles<scalar, ippl::fdtd_bc::absorbing> field_state(layout, mesh,
                                                                                pcount);
    ...
    typename ippl::FELSimulationState<scalar>::e_view_type::host_mirror_type view =
        Kokkos::create_mirror_view(field_state.E.getView());
    for (unsigned i = 1; i < nr[2]; i++) {
        ...
        line << pos.norm() * unit_length_in_meters << " "
             << ebg.norm() * unit_electric_fieldstrength_in_voltpermeters << "\n";
    }
    return 0.0f;
}
template <typename scalar>
scalar test_amperes_law(uint32_t n) {
    ippl::NDIndex<3> owned(n / 2, n / 2, 2 * n);
    ippl::Vector<uint32_t, 3> nr{n / 2, n / 2, 2 * n};
    ippl::Vector<scalar, 3> extents{meter_in_unit_lengths, (scalar)(4 * meter_in_unit_lengths),
                                    meter_in_unit_lengths};
    std::array<bool, 3> isParallel;
    isParallel.fill(false);
    isParallel[2] = true;
    ...
    using vector_type              = ippl::Vector<scalar, 3>;
    ippl::Vector<scalar, 3> origin = {scalar(-0.5 * meter_in_unit_lengths),
                                      scalar(-2.0 * meter_in_unit_lengths),
                                      scalar(-0.5 * meter_in_unit_lengths)};
    ippl::UniformCartesian<scalar, 3> mesh(owned, hx, origin);
    ...
    typename ippl::FELSimulationState<scalar>::b_view_type::host_mirror_type view =
        Kokkos::create_mirror_view(field_state.B.getView());
    // ippl::Vector<ippl::Vector<scalar, 3>, 2> ebg = gather_helper(view, ippl::Vector<scalar,
    // 3>{0,0,0}, origin, hx, lDom);
    for (unsigned i = 1; i < nr[2]; i++) {
        vector_type pos = {scalar(0), scalar(0), (scalar)origin[2]};
        pos[2] += hx[2] * scalar(i);
        ippl::Vector<scalar, 3> ebg = gather_helper(view, pos, origin, hx, lDom);
        // line << pos.norm() * unit_length_in_meters << " " << (view(n / 4, n / 4, i)[0].norm()) *
        // unit_electric_fieldstrength_in_voltpermeters << "\n";
        line << pos.norm() * unit_length_in_meters << " "
             << ebg[0] * unit_magnetic_fluxdensity_in_tesla << "\n";
    }
    return 0.0f;
}
int main(int argc, char* argv[]) {
    ...
    ippl::initialize(argc, argv);
    {
        ...
        const scalar frame_gamma = std::max(
            ...
            cfg.bunch_gamma
                / std::sqrt(1.0 + cfg.undulator_K * cfg.undulator_K * config::scalar(0.5)));
        ...
        ippl::UniaxialLorentzframe<scalar, 2> frame_boost(frame_gammabeta);
        ippl::undulator_parameters<scalar> uparams(cfg.undulator_K, cfg.undulator_period,
                                                   cfg.undulator_length);
        ...
        //[-1, 1] box
        ippl::Vector<scalar, 3> hx     = {scalar(cfg.extents[0] / cfg.resolution[0]),
                                          scalar(cfg.extents[1] / cfg.resolution[1]),
                                          scalar(cfg.extents[2] / cfg.resolution[2])};
        ippl::Vector<scalar, 3> origin = {scalar(-cfg.extents[0] * 0.5),
                                          scalar(-cfg.extents[1] * 0.5),
                                          scalar(-cfg.extents[2] * 0.5)};
        ippl::UniformCartesian<scalar, 3> mesh(owned, hx, origin);
        std::cout << "hx: " << hx << "\n";
        ...
        if (ippl::Comm->rank() == 0) {
            ...
            size_t actual_pc = initialize_bunch_mithra(fdtd_state.fieldsAndParticles.particles,
                                                       mithra_config, frame_gamma);
        ...
        }
        {
            ...
            ippl::Vector<scalar, 3> meanpos =
                fdtd_state.fieldsAndParticles.particles.R.sum()
                * (1.0 / fdtd_state.fieldsAndParticles.particles.getTotalNum());

            Kokkos::parallel_for(
                fdtd_state.fieldsAndParticles.particles.getLocalNum(), KOKKOS_LAMBDA(size_t i) {
                    rview(i) -= meanpos;
                    rm1view(i) -= meanpos;
                });
        }
        ...
        if (ippl::Comm->rank() == 0) {
            ...
            const char* ffmpegCmd =
                "ffmpeg -y -f image2pipe -vcodec bmp -r 30 -i - -vf "
                "scale=force_original_aspect_ratio=decrease:force_divisible_by=2,format=yuv420p "
                "-c:v libx264 -movflags +faststart ffmpeg_popen.mkv";
        ...
        for (size_t i = 0; i < timesteps_required; i++) {
            ...
            Kokkos::parallel_reduce(
                ...
                KOKKOS_LAMBDA(uint32_t i, uint32_t j, uint32_t k, double& ref) {
                    Kokkos::pair<ippl::Vector<scalar, 3>, ippl::Vector<scalar, 3>> buncheb{
                        eview(i, j, k), bview(i, j, k)};
                ...
                radiation);
            double radiation_in_watt_on_this_rank =
                radiation
            ...
            MPI_Reduce(&radiation_in_watt_on_this_rank, &radiation_in_watt_global, 1, MPI_DOUBLE,
            ...
            if ((cfg.output_rhythm != 0) && (i % cfg.output_rhythm == 0)) {
                ...
                for (size_t hi = 0; hi < fdtd_state.fieldsAndParticles.particles.getLocalNum();
                     hi++) {
                    ...
                    if (y_imgcoord >= 0 && x_imgcoord >= 0 && x_imgcoord < img_width
                        && y_imgcoord < img_height) {
                        const float intensity =
                            std::min(255.f, (img_width * img_height * 15.f) / cfg.num_particles);
                        // std::cout << intensity << "\n";
                        imagedata[(y_imgcoord * img_width + x_imgcoord) * 3 + 1] =
                            std::min(255.f, imagedata[(y_imgcoord * img_width + x_imgcoord) * 3 + 1]
                                                + intensity);
                    }
                };
...

                {
                    for (int i = 1; i < img_width; i++) {
                        for (int j = 1; j < img_height; j++) {
                            int i_remap =
                                (double(i) / (img_width - 1)) * (fdtd_state.nr_global[2] - 4) + 2;
                            int j_remap =
                                (double(j) / (img_height - 1)) * (fdtd_state.nr_global[0] - 4) + 2;
                            if (i_remap >= ldom.first()[2] && i_remap <= ldom.last()[2]) {
                                if (j_remap >= ldom.first()[0] && j_remap <= ldom.last()[0]) {
                                    ippl::Vector<scalar, 3> E = eh(j_remap + 1 - ldom.first()[0],
                                                                   fdtd_state.nr_global[1] / 2,
                                                                   i_remap + 1 - ldom.first()[2]);
                                    ippl::Vector<scalar, 3> B = bh(j_remap + 1 - ldom.first()[0],
                                                                   fdtd_state.nr_global[1] / 2,
                                                                   i_remap + 1 - ldom.first()[2]);

                                    ippl::Vector<scalar, 3> poynting = E.cross(B);
                                    Kokkos::pair<ippl::Vector<scalar, 3>, ippl::Vector<scalar, 3>>
                                        eblab = frame_boost.inverse_transform_EB(
                                            Kokkos::make_pair<ippl::Vector<scalar, 3>,
                                                              ippl::Vector<scalar, 3>>(E, B));
                                    ippl::Vector<scalar, 3> poyntinglab =
                                        eblab.first.cross(eblab.second);
                                    poynting = poyntinglab;
                                    if (poynting.norm() > 0) {
                                        // std::cout << poynting.norm() << "\n";
                                    }
                                    float normalized_colorscale_value =
                                        std::sqrt(poynting.norm()) * 0.00001f;
                                    int index = (int)std::max(
                                        0.0f,
                                        std::min(normalized_colorscale_value * 255.0f, 255.0f));
                                    imagedata[(j * img_width + i) * 3 + 0] +=
                                        turbo_cm[index][0]
                                        * 255.0f;  // * std::min(normalized_colorscale_value *
                                                   // 100.0f + 50.0f, 150.0f);//(unsigned
                                                   // char)(std::min(255u, (unsigned int)(0.5f *
                                                   // std::sqrt(std::sqrt(poynting.squaredNorm())))));
                ...
                /*
                 * This code should be replaced by the Image Gathering anyway, but that's a
                 * different PR
                 */
                int mask = 1;
                while (mask < size) {
                    int partner = rank ^ mask;
                    {
                        if ((rank & mask) == 0) {
                            ...
                            MPI_Recv(idata_recvbuffer, floatcount, MPI_FLOAT, partner, 0,
                                     ippl::Comm->getCommunicator(), MPI_STATUS_IGNORE);
                            // Apply image summation
                            for (int f = 0; f < floatcount; f++) {
                                imagedata[f] += idata_recvbuffer[f];
                            }
                        } else {
                            MPI_Send(imagedata, floatcount, MPI_FLOAT, partner, 0,
                ...
                if (rank == 0) {
                    ...
                    std::transform(imagedata, imagedata + img_height * img_width * 3,
                                   imagedata_final, [](float x) {
        ...
        if (ippl::Comm->rank() == 0)
            std::cout << ippl::Comm->size() << " " << double(endtime - starttime) / 1e9
                      << std::endl;
    }
    // exit:
    ippl::finalize();
