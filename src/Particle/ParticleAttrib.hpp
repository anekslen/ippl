...
namespace ippl {
    ...
    void ParticleAttrib<T, Properties...>::gatherCommon(
        Field& f, const ParticleAttrib<Vector<P2, Field::dim>, Properties...>& pp,
        const typename Field::Mesh_t::vector_type::value_type offset, const bool addToAttribute) {
        constexpr unsigned Dim = Field::dim;
        using PositionType     = typename Field::Mesh_t::value_type;
    ...
    template <typename T, class... Properties>
    template <typename Field, typename P2>
    void ParticleAttrib<T, Properties...>::gather(Field& f, const ParticleAttrib<Vector<P2, Field::dim>, Properties...>& pp,
        const bool addToAttribute) {

        // Call gather with cell centered data -> offset 0.5
        this->gatherCommon(f, pp, 0.5, addToAttribute);
    }

    template <typename T, class... Properties>
    template <typename Field, typename P2>
    void ParticleAttrib<T, Properties...>::gatherFromNodes(Field& f, const ParticleAttrib<Vector<P2, Field::dim>, Properties...>& pp,
        const bool addToAttribute) {

        // Call gather with node centered data -> offset 0.0
        this->gatherCommon(f, pp, 0.0, addToAttribute);
    }
    ...
    /**
     * @brief Non-class interface for gathering field data from cell centers into a particle attribute.
     * 
    ...
    /**
     * @brief Non-class interface for gathering field data from nodes into a particle attribute.
     * 
     * This interface calls the member ParticleAttrib::gatherFromNodes() function with the provided
     * parameters and preserving legacy behavior by assigning `addToAttribute` a default value.
     * 
     * @note See ParticleAttrib::gatherFromNodes() for more information on the behavior of `addToAttribute`.
     * 
     * @tparam Attrib1 The type of the particle attribute.
     * @tparam Field The type of the field.
     * @tparam Attrib2 The type of the particle position attribute.
     ...
     */
    template <typename Attrib1, typename Field, typename Attrib2>
    inline void gatherFromNodes(Attrib1& attrib, Field& f, const Attrib2& pp, 
                        const bool addToAttribute = false) {
        attrib.gatherFromNodes(f, pp, addToAttribute);
    }
