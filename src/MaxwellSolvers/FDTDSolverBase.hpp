...
namespace ippl {
    ...
    /**
     * @brief Gets the view of the A field at the current time step.
     * 
     * @return The view of the A field at the current time step.
     */
    template <typename EMField, typename SourceField, fdtd_bc boundary_conditions>
    auto FDTDSolverBase<EMField, SourceField, boundary_conditions>::getAView() {
        return A_n.getView();
    }
    
        /**
         * @brief Gets the view of the A field at the next time step.
         * 
         * @return The view of the A field at the next time step.
         */
        template <typename EMField, typename SourceField, fdtd_bc boundary_conditions>
        auto FDTDSolverBase<EMField, SourceField, boundary_conditions>::getANextView() {
            return A_np1.getView();
        }

    /**
     * @brief Gets the view of the A field at the previous time step.
     * 
     * @return The view of the A field at the previous time step.
     */
    template <typename EMField, typename SourceField, fdtd_bc boundary_conditions>
    auto FDTDSolverBase<EMField, SourceField, boundary_conditions>::getAPrevView() {
        return A_nm1.getView();
    }
