...
namespace ippl {
     ...
     *
     * This constructor initializes the NonStandardFDTDSolver with the given source field and electromagnetic fields.
     * It checks the dispersion-free CFL condition and initializes the solver.
     *
    ...
    void NonStandardFDTDSolver<EMField, SourceField, boundary_conditions>::step() {
        ...
        const scalar calA = 0.25 * (1 + 0.02 / (static_cast<scalar>(Kokkos::pow(this->hr_m[2] / this->hr_m[0], 2)) + static_cast<scalar>(Kokkos::pow(this->hr_m[2] / this->hr_m[1], 2))));
        nondispersive<scalar> ndisp{
            .a1 = 2 * (1 - (1 - 2 * calA) * static_cast<scalar>(Kokkos::pow(this->dt / this->hr_m[0], 2))
            ...
            .a6 = static_cast<scalar>(Kokkos::pow(this->dt / this->hr_m[2], 2)) - 2 * calA * static_cast<scalar>(Kokkos::pow(this->dt / this->hr_m[0], 2)) - 2 * calA * static_cast<scalar>(Kokkos::pow(this->dt / this->hr_m[1], 2)),
