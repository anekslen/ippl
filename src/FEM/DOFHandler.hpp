...
namespace ippl {
    ...
    void DOFHandler<T, SpaceTag, Dim, Order>::initialize(Mesh_t& mesh, const Layout_t& layout) {
        ...
        ne_m[d] = mesh.getGridsize() - 1;

        // Allocate DualView for DOF mapping table
        dofMappingTable_m = Kokkos::DualView<DOFMapping*>("DOFMappingTable", dofsPerElement);

        // Fill the mapping table on host
        auto h_table = dofMappingTable_m.h_view;
        ...
        dofMappingTable_m.modify_host();
        dofMappingTable_m.sync_device();
    }
    
    template <typename T, typename SpaceTag, unsigned Dim, unsigned Order>
    void DOFHandler<T, SpaceTag, Dim, Order>::fillLagrangeDOFMappingTable(typename Kokkos::View<DOFMapping*>::HostMirror& hostTable) const {
        size_t dofIndex = 0;
        ...
        if constexpr (Dim >= 2) {
            ...
            if constexpr (TagIndex<EntityTypes>::template contains<EdgeX<Dim>>()) {
                ...
                constexpr size_t numEdgesX = (Dim == 2) ? 2 : 4;
    ...
    template <typename T, typename SpaceTag, unsigned Dim, unsigned Order>
    void DOFHandler<T, SpaceTag, Dim, Order>::fillNedelecDOFMappingTable(typename Kokkos::View<DOFMapping*>::HostMirror& hostTable) const {
    ...
    DOFHandler<T, SpaceTag, Dim, Order>::getElementDOFMapping(const size_t& localElementDOF) const {
        // Use view() method which automatically selects the right view based on execution space
        // This is resolved at compile time based on where the function is instantiated
        return dofMappingTable_m.view<typename Kokkos::DefaultExecutionSpace::memory_space>()(localElementDOF);
    }
